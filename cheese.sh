...
INSTALL_TRAY_PATH="$INSTALL_BIN_DIR/cheese-tray"
...
HYPR_SNIPPET="$HYPR_DIR/cheese-bindings.conf"
...
EOF
  ...
  if [[ -f "$HYPR_CONFIG" ]]; then
    local source_line="source = ./cheese-bindings.conf"
    if ! grep -Fq "$source_line" "$HYPR_CONFIG"; then
      printf '\n%s # %s capture shortcuts and window rules\n' "$source_line" "$APP_TITLE" >> "$HYPR_CONFIG"
    fi
  else
    cat > "$HYPR_CONFIG" <<EOF
# Generated by $APP_TITLE installer.
